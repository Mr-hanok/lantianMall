    ...
    for (NSUInteger i = 0 ; i < titlesArray.count; i++) {
        ...
        if (_titlesBarStyles == 0) {
            ...
            if (_sizeToFitIsEnabled && (_heightToFitIsEnabled || _widthToFitIsEnabled)) {
                ...
                /*非要加个hot标题*/
                if (self.isHaveSubtitle) {
                    if (i==1) {
                        UIImageView *hotlabel = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"chihuohot"]];
                        hotlabel.frame = CGRectMake(tabbutton.jc_width-12, 5, 24, 8);
                        [hotlabel sizeToFit];
                        [tabbutton addSubview:hotlabel];
                    }
                }
